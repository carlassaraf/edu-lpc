...
SPI::SPI(uint32_t spin, uint32_t mosi, uint32_t miso, uint32_t sck, uint32_t ssel) {
    ...
    if(spin == 0) {
    	spi = SPI0;
        SWM_SetMovablePinSelect(SWM0, kSWM_SPI0_SCK, (swm_port_pin_type_t)sck);
        SWM_SetMovablePinSelect(SWM0, kSWM_SPI0_MOSI, (swm_port_pin_type_t)mosi);
        SWM_SetMovablePinSelect(SWM0, kSWM_SPI0_MISO, (swm_port_pin_type_t)miso);
        SWM_SetMovablePinSelect(SWM0, kSWM_SPI0_SSEL0, (swm_port_pin_type_t)ssel);
        CLOCK_Select(kSPI0_Clk_From_MainClk);
    }
    else if(spin == 1) {
    	spi = SPI1;
        SWM_SetMovablePinSelect(SWM0, kSWM_SPI1_SCK, (swm_port_pin_type_t)sck);
        SWM_SetMovablePinSelect(SWM0, kSWM_SPI1_MOSI, (swm_port_pin_type_t)mosi);
        SWM_SetMovablePinSelect(SWM0, kSWM_SPI1_MISO, (swm_port_pin_type_t)miso);
        SWM_SetMovablePinSelect(SWM0, kSWM_SPI1_SSEL0, (swm_port_pin_type_t)ssel);
        CLOCK_Select(kSPI0_Clk_From_MainClk);
    }
